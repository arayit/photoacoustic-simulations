...
% --- Reference scenarios ---
s     = load(fullfile(results_dir, 's01_ns_tau3ns_F10.mat'),   'results');
p_s01 = max(abs(s.results.sensor_data(:)));

s     = load(fullfile(results_dir, 's02_fs_tau100fs_F1.mat'),  'results');
p_s02 = max(abs(s.results.sensor_data(:)));
...
plot(N_list, p_burst, 'b-o', 'LineWidth', 1.5, 'MarkerSize', 5, ...
    'DisplayName', 'FS burst  (F_p = 1 J/cm², \tau_p = 100 fs)');
yline(p_s01, 'r--', 'LineWidth', 1.5, ...
    'DisplayName', sprintf('NS single pulse  (F = 10 J/cm², \\tau = 3 ns) — %.3g Pa', p_s01));
yline(p_s02, 'k--', 'LineWidth', 1.5, ...
    'DisplayName', sprintf('FS single pulse  (F = 1 J/cm², \\tau = 100 fs) — %.3g Pa', p_s02));
